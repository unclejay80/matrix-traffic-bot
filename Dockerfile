...
RUN mkdir -p /src/traffic_bot
COPY traffic_bot/__init__.py /src/traffic_bot/
COPY README.md traffic-bot /src/
...
COPY *.py *.md /src/
COPY traffic_bot/*.py /src/traffic_bot/
...
# Start the bot
ENTRYPOINT ["traffic-bot", "/data/config.yaml"]
